...
function listScores(){
    console.log("invoked scores.list");
    ...
    fetch(url, {
        ...
        if (response.hasOwnProperty("Error")){
            ...
            formatScoresList(response);
        }
    });
}

function formatScoresList(JSONArray){
    //creates a new string for the data to be processed into html
    let dataHTML = "<tr><th>Username</th><th>Score</th></tr>";
    //adds the values to dataHTML and formats them as a table
    for(let item of JSONArray){
        dataHTML += "<tr><td>" + item.UserName + "</td><td>" + item.ScoreValue + "</td></tr>"
    }
    //adds the html to the ScoresTable element
    document.getElementById("scoresTable").innerHTML = dataHTML;
}

function listTimes(){
    console.log("invoked times.list");
    const url = "/times/list";
    fetch(url, {
        method:"GET",
        //returns the JSON received
    }).then(response =>{
        ...
        if (response.hasOwnProperty("Error")){
            ...
            formatTimesList(response);
        }
    });
}

function formatTimesList(JSONArray){
    //creates a new string for the data to be processed into html
    let dataHTML = "<tr><th>Username</th><th>Time</th></tr>";
    //adds the values to dataHTML and formats them as a table
    for(let item of JSONArray){
        dataHTML += "<tr><td>" + item.UserName + "</td><td>" + item.TimeValue + "</td></tr>"
    }
    //adds the html to the TimesTable element
    document.getElementById("timesTable").innerHTML = dataHTML;
}
